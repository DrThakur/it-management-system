...
const AssignedTicketTable = ({ user }) => {
    ...
    const fetchTickets = async () => {
      try {
        const res = await axios.get(
          `http://${baseURL}:${port}/tickets/assigned/${user._id}`
        );
  ...
  const handleTicketAssignment = async (ticketObjId) => {
    ...
    try {
      ...
      toast.success(`Ticket ${ticketObjId} assigned successfully!`, {
